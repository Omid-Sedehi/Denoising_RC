...
if __name__ == "__main__":
    ...
    x_true = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\X_true.txt')
    X_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\X_train.txt')
    y_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\y_train.txt')
    Time_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\Time_train.txt')
    X_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\X_test.txt')
    y_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\y_test.txt')
    Time_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\Time_test.txt')
    Time = np.loadtxt('.\Dataset\AINF_Dataset10p_Violet\Time.txt')
    ...
    torch.save(rc, './Models/Model_Tuned_10p_Violet.pth')
...
####### Perform Pruning ###########################################################################################
    ...
    # Save the entire model
    torch.save(rc, './Models/Model_Pruned_10p_Violet.pth')
    ...
    axs[0].text(0.02, 0.95, 'd)', transform=axs[0].transAxes, fontsize=12, verticalalignment='top')

    # Subplot 2
    axs[1].plot(1000 * (x_true[100:, 0] - X_Total[100:, 0]), 1e12 * (x_true[100:, 1] - X_Total[100:, 1]), linestyle='--', color='seagreen', label='Original', linewidth=1)
    axs[1].plot(1000 * (x_true[100:, 0] - y_pred_tuned[100:, 0]), 1e12 * (x_true[100:, 1] - y_pred_tuned[100:, 1]), linestyle='--', color='firebrick', label='Residual', linewidth=1)
    ...
    axs[1].text(0.02, 0.95, 'e)', transform=axs[1].transAxes, fontsize=12, verticalalignment='top')
    ...
    axs[2].text(0.02, 0.95, 'f)', transform=axs[2].transAxes, fontsize=12, verticalalignment='top')
    ...
    plt.style.use('./Styles/AIPStyles.mplstyle')
    fig, axs = plt.subplots(figsize=(5.00, 2.75))
    fig.subplots_adjust(wspace=0.4, hspace=0.5)  # `wspace` adjusts width, `hspace` adjusts height
    # Subplot 3 (Power Spectral Density - PSD)
    NFFT = 512  # Length of each segment
    axs.psd(1000 * (x_true[100:, 0]), NFFT=NFFT, noverlap=NFFT // 2, Fs=100000, color='#4682B4', label=r'Ground Truth')
    axs.psd(1000 * (X_Total[100:, 0]), NFFT=NFFT, noverlap=NFFT // 2, Fs=100000, color='seagreen', label=r'Noisy')
    axs.psd(1000 * (y_pred_tuned[100:, 0]), NFFT=NFFT, noverlap=NFFT // 2, Fs=100000, color='firebrick', label=r'Denoised')
    axs.set_xlabel(r'$f$ (kHz)')
    axs.set_ylabel(r'$S_V (f)$ (dB/Hz)')
    axs.set_xlim([0, 50])
    axs.grid(False)
    axs.legend(loc='upper center', bbox_to_anchor=(0.5, 1.20), ncol=3)
    axs.text(0.02, 0.95, 'b)', transform=axs.transAxes, fontsize=12, verticalalignment='top')
    axs.set_xticks(ticks=[0, 10000, 20000, 30000, 40000, 50000], labels=[0, 10, 20, 30, 40, 50])
    fig.tight_layout()
    fig.show()