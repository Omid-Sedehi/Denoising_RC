...
if __name__ == "__main__":
    ...
    torch.save(rc, './Models/Model_Tuned_10p_GWN.pth')
...
####### Perform Pruning ###########################################################################################
    ...
    # Save the entire model
    torch.save(rc, './Models/Model_Pruned_10p_GWN.pth')
    ...
    axs.set_ylabel(r'$S_{V} (f)$ (dB/Hz)')
    axs.set_xlim([0, 50])
    axs.grid(False)
    axs.legend(loc='upper center', bbox_to_anchor=(0.5, 1.20), ncol=3)
    axs.text(0.02, 0.95, 'a)', transform=axs.transAxes, fontsize=12, verticalalignment='top')
