...
if __name__ == "__main__":
    ...
    x_true = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\X_true.txt')
    X_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\X_train.txt')
    y_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\y_train.txt')
    Time_train = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\Time_train.txt')
    X_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\X_test.txt')
    y_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\y_test.txt')
    Time_test = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\Time_test.txt')
    Time = np.loadtxt('.\Dataset\AINF_Dataset10p_Brown\Time.txt')
    ...
    torch.save(rc, './Models/Model_Tuned_10p_Brown.pth')
...
####### Perform Pruning ###########################################################################################
    ...
    # Save the entire model
    torch.save(rc, './Models/Model_Pruned_10p_Brown.pth')
    ...
    # Subplot 1
    axs[0].plot(1000 * X_Total[100:, 0], 1000 * X_Total[100:, 1], linestyle='--', color='seagreen', label='Input', linewidth=1)
    axs[0].plot(1000 * y_pred_tuned[100:, 0], 1000 * y_pred_tuned[100:, 1], linestyle='--', color='firebrick', label='Output', linewidth=1)
    ...
    axs[0].text(0.02, 0.95, 'j)', transform=axs[0].transAxes, fontsize=12, verticalalignment='top')

    # Subplot 2
    axs[1].plot(1000 * (x_true[100:, 0] - X_Total[100:, 0]), 1000 * (x_true[100:, 1] - X_Total[100:, 1]), linestyle='--', color='seagreen', label='Original', linewidth=1)
    axs[1].plot(1000 * (x_true[100:, 0] - y_pred_tuned[100:, 0]), 1000 * (x_true[100:, 1] - y_pred_tuned[100:, 1]), linestyle='--', color='firebrick', label='Residual', linewidth=1)
    ...
    axs[1].text(0.02, 0.95, 'k)', transform=axs[1].transAxes, fontsize=12, verticalalignment='top')

    # Subplot 3 (Power Spectral Density - PSD)
    NFFT = 512  # Length of each segment
    axs[2].psd((x_true[100:, 0] - X_Total[100:, 0]), NFFT=NFFT, noverlap=NFFT // 2, Fs=100000, color='seagreen', label=r'Original')
    axs[2].psd((x_true[100:, 0] - y_pred_tuned[100:, 0]), NFFT=NFFT, noverlap=NFFT // 2, Fs=100000, color='firebrick', label=r'Residual')
    ...
    axs[2].text(0.02, 0.95, 'l)', transform=axs[2].transAxes, fontsize=12, verticalalignment='top')
    ...
    ax_inset.set_xticks(ticks=[0, 500, 1000], labels=[0, 0.5, 1])
